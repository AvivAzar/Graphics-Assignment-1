...
glm::vec3 cap(glm::vec3 vec) { //caps vector values between 0 and 1, used for color vectors
	if (vec.x < 0) vec.x = 0;
	if (vec.y < 0) vec.y = 0;
	if (vec.z < 0) vec.z = 0;
	if (vec.x > 1) vec.x = 1;
...
float sphereIntersection(glm::vec3 p0, glm::vec3 v, glm::vec4 sphere) {//p0-ray origin (camera if ray hasn't bounced yet), v-curret ray, sphere- sphere currently checked for intersection
...
float planeIntersection(glm::vec3 p0, glm::vec3 v, glm::vec4 plane) {//p0-ray origin (camera if ray hasn't bounced yet), v-curret ray, plane- plane currently checked for intersection
...
bool isLightBlockedBy(glm::vec3 p, glm::vec3 dir, glm::vec4 sphere, int light) {
	if (!isSphere(sphere)) return false;
...
glm::vec3 getObjectNormal(int i, glm::vec3 p) {
	return normalize(isSphere(scn.objects[i]) ? glm::vec3(scn.objects[i].x - p.x, scn.objects[i].y - p.y, scn.objects[i].z - p.z) : make3(scn.objects[i]));
}
...
glm::vec3 colorCalc(glm::vec3 intersectionPoint, glm::vec3 v, int raysLeft, int prevObject)
{
	...
	if (scn.objectProps[intersection].x < 1 && scn.objectProps[intersection].y < 1) {
		...
		// Diffuse:
		result += applyDiffuse(intersection, p, N, intersectionColor);
		// Specular:
